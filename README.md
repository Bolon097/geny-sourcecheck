...
1. Level 1: whether an AI response contains explicit source signals, including URLs, DOI references, bare domains, or source-related wording.
2. Level 2: whether extracted URLs, domains, or DOI links can be reached from the user's own computer through the local Python backend.
...
Pasted AI responses are sent only to the local backend at `127.0.0.1:8000`. They are not sent to the project author, OpenAI, or any other LLM API. During Level 2 checks, the user's own computer requests the cited websites, domains, or DOI resolver pages.
...
- Expected Level 2: try `https://data.hangzhou.gov.cn`, then `http://data.hangzhou.gov.cn` if needed

Test 2: `Data from （data.zjzwfw.gov.cn）`

- Expected: extract `data.zjzwfw.gov.cn`
- Expected: strip Chinese brackets
- Expected source type: `bare_domain`

Test 3: `See https://www.iea.org/reports/global-ev-outlook-2024`

- Expected: extract full URL
- Expected source type: `url`

Test 4: `doi:10.1016/j.apenergy.2023.xxxxxx`

- Expected: extract DOI
- Expected normalized source: `https://doi.org/10.1016/j.apenergy.2023.xxxxxx`
- Expected source type: `doi`

Test 5: `According to government data, EV charging demand is increasing.`
...
- Expected Level 2: not checkable

Test 6: `This is a model answer with no citation.`

- Expected Level 1: failed
- Expected source count: `0`

Test 7: `链接：(zjjcmspublic.oss-cn-hangzhou-zwynet-d01-a.internet.cloud.zj.gov.cn)`

- Expected: extract the full bare domain
- Expected: strip brackets
- Expected Level 2: try HTTPS first, then HTTP if needed
